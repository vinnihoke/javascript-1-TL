 ...
 *
 * @instructions
 * This function takes name as its only argument
...
function sortCarInventory(inventory) {
  return inventory.sort(function(a, b) {
    var currentCar = a.car_model.toLowerCase();
    var nextCar = b.car_model.toLowerCase();
    if (currentCar > nextCar) {
      return 1;
    } else if (currentCar < nextCar) {
      return -1;
    } else {
      return 0;
    }
...
function getOlderCars(inventory, maxYear) {
  // Returns an array containing all the cars < maxYear
  let oldCars = [];
  // Loop over inventory,
  for (let i = 0; i < inventory.length; i++) {
    // push into oldCars if < maxYear
    ...
    // Ternary
    inventory[i].car_year <= maxYear ? oldCars.push(inventory[i]) : null;
  }
  return oldCars;
}
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    // inventory[i].car_make === "Audi" ? germanCars.push(inventory[i]) : null
    // inventory[i].car_make === "Mercedes-Benz" ? germanCars.push(inventory[i]) : null
    // inventory[i].car_make === "Volkswagen" ? germanCars.push(inventory[i]) : null
    // inventory[i].car_make === "BMW" ? germanCars.push(inventory[i]) : null
  }
  return germanCars;
}
